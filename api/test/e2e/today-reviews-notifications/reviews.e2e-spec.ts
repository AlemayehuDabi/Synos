...
describe('Reviews (e2e)', () => {
  ...
  describe('generating a user\'s reviews', () => {
    ...
    it('has one section per registered domain, then the cross-domain one, and a bad domain never fails the review', async () => {
      ...
      expect(weekly.sections[0]).toEqual({ domain: 'tasks', status: 'ok', metrics: { completed: 0 }, highlights: [] });
      expect(weekly.sections[1]).toEqual({ domain: 'habits', status: 'error' });
      expect(weekly.sections[2]).toEqual({ domain: 'finances', status: 'timeout' });
