...
describe('Signal engine: suggest mode (e2e)', () => {
  ...
  it('creates a pending suggestion, approves it, logs activity, and can be undone', async () => {
    const { signal, payload } = await emitWorkoutCompleted(facade, userA.userId);

    const inboxRes = await request(app.getHttpServer())
      .get('/api/v1/inbox')
      .query({ connectionId: 'workout-to-habit' })
      .set(bearer(userA.token))
      .expect(200);
    const created = inboxRes.body.items.find((s: { targetKey: string }) => s.targetKey === `sandbox:workout:${payload.workoutId}`);
    ...
    const detail = await request(app.getHttpServer())
      ...
      .set(bearer(userA.token))
      .expect(200);
    expect(detail.body.trigger).toMatchObject({ signalId: signal.id, type: 'workout.completed' });
    ...
    expect(approved.body.revertData).toEqual({ workoutId: payload.workoutId });
    ...
    expect(approvedActivity.after).toEqual({ reminderFor: payload.workoutId });
  ...
  it('rejects approving with params that fail the handler schema (422)', async () => {
    const { payload } = await emitWorkoutCompleted(facade, userA.userId);
    const inboxRes = await request(app.getHttpServer())
      .get('/api/v1/inbox')
      .set(bearer(userA.token))
      .expect(200);
